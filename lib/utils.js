...
function getConnectionOptions(app) {
  ...
  const entitiesArr = typeorm.entities || [];
  return clientArr.map(e => {
    const ed = e.entitiesDir;
    // 防止client定义时没有name属性，以及没有定义name属性值给予default
    e.name = e.name || 'default';
    for (const entityArrItem of entitiesArr) {
      // 匹配根据名称匹配实体类存放目录，请勿重名
      if (e.name === entityArrItem.name) {
        e.entitiesDir = entityArrItem.entitiesDir;
        break;
      }
    }
    if (!e.entitiesDir) {
    ...
    e.entities = [ path.join(app.config.baseDir, ed, '**', isLocal ? '*.ts' : '*.js') ];
    return e;
  });
}
