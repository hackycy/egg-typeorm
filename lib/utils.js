...
function isProdEnv(app) {
  ...
  if (typeorm && typeorm.prodEnv) {
    const { prodEnv } = typeorm;
    if (Array.isArray(prodEnv)) {
      return prodEnv.includes(app.config.env);
    }
    return app.config.env === prodEnv;
  }
...
module.exports = { getConnectionOptions, loadOrmConfig, camelize, parse, load, toHump, isProdEnv };
