...
async function i(i, c, s, a) {
  ...
  if (!g && !c) return;
  const y = n.sync(o.join(l, `${g}/**/${i.config.env === 'local' ? '*.ts' : '*.js'}`)).map(e => o.relative(l, e).replace(/\\/gi, '/'));
  try {
    for (const n of y) {
...
exports.closeConnections = async function() {
  ...
  await async function(e, t) {
    ...
    for (const o of t) {
      ...
      await i(e, o, n, r), t === 'default' && (Object.keys(n).forEach(t => {
  ...
  for (const e of n) {
    ...
    o && o.length && +o[0].result == 2 && t.logger.info(`[egg-typeorm] connection ${e.name} is ready`);
  }
};
